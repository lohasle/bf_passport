package com.sz.weizy.passport.utils.dz;
...
public class UcAuthCode {
     ...
     * @return
     */
    public static String generateToken(String userName,String pwd,String key,String time){
        //1 结构 用户名\t+时间戳\t+密码
        String str = userName+"\t"+(new Date().getTime())+"\t"+pwd;
        //2 authcode 加密
        int lt = ("".equals(time)||null==time)?0:Integer.parseInt(time);
        String token = null;
        try {
            token = URLEncoder.encode(UcAuthCode.authcodeEncode(str, key, lt), "UTF-8");
     ...
     * 解密token 取得用户名和密码
     * @param token
     * @return  nameAndPwd  or null（没拿到）
     */
    public static String[] getNameAndPwd(String token,String key){
        String[] nameAndPwd = null;
        String authStr = null;
        try {
            authStr = authcodeDecode(URLDecoder.decode(token, "utf-8"),key);
            String[] authStrArr = authStr.split("\t");
            if(authStrArr.length==3){
                nameAndPwd =  new String[2];
                //解密之后的数据
                nameAndPwd[0]= authStrArr[0];
                nameAndPwd[1]= authStrArr[2];
            }
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
    ...
    @Deprecated
    public static String generateToken(String str,String key){
        //1 加密字符
        //2 authcode 加密
        String token = null;
        try {
            token = URLEncoder.encode(UcAuthCode.authcodeEncode(str, key), "utf-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        return token;
